...
# This one doesn't workkkkk  >>>puts journalistes.count { |m| m=~ /1, 2, 3, 4, 5, 6, 7, 8, 9/}
...
journalistes.each do |item| 
	...
	puts "Il y a #{c} handle qui contiennent un numéro"
#17 handle qui contiennent un numéro 
...
puts "#{c}"
#Il y a 209 noms qui commencent par une majuscule


puts "Combien contiennent une majuscule ?"

# This one Doesn't work well >>>  puts journalistes.count { |m| m=~ /[A-Z]/}
...
#215 contiennent une majuscule 

puts "Combien de _ dans tous les pseudos confondus ?"

# This one is wrong because it count every name with underscore but not every  underscore >>>>> puts journalistes.count { |m| m=~ /_/}
...
puts journalistes.sort_by {|alpha| alpha.downcase}


puts "Combien d'handle contiennent les 4 lettres du prénom Aude à la suite (sans faire attention à la casse) ?"

journalistes.map!(&:downcase)
#utiliser pour tout mettre en minuscule

puts journalistes.count { |m| m=~ /aude/}
#3 fois le mot 'aude' dans l'ensemble des journalistes